...
public class WebSocketSession : IDisposable
{
    ...
    private static void SendMessage(TcpClient client, string payload, bool masking = false) =>
                SendMessage(client, Encoding.UTF8.GetBytes(payload), false, masking);

    private static void SendMessage(TcpClient client, byte[] payload, bool isBinary = false, bool masking = false)
    {
        var mask = new byte[4];
        if (masking)
        {
    ...
    private static void SendMessage(TcpClient client, byte[] payload, int opcode, bool masking, byte[] mask)
    {
    ...
    private static byte[] ApplyMask(IReadOnlyList<byte> msg, IReadOnlyList<byte> mask)
    {
